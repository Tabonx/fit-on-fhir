...
namespace Microsoft.Health.FitOnFhir.Common.ExtensionMethods
{
    ...
    public static class DependencyInjectionExtensions
    {
        ...
        public static IResponsibilityHandler<TRequest, TResult> CreateOrderedHandlerChain<TRequest, TResult>(
            ...
            where TResult : class
        {
            if (handlerTypes.IsNullOrEmpty())
            {
                throw new ArgumentException("handlers empty or wrong type");
            }
            else if (handlerTypes.Length != 0 && handlerTypes.All(handler => handler.GetInterfaces().Contains(typeof(IResponsibilityHandler<TRequest, TResult>))))
            {
                ...
                foreach (Type handlerType in handlerTypes.Reverse())
                {
                    ...
                    {
                        previousHandler = handler.Chain(previousHandler);
                    }
                    else
                    {
